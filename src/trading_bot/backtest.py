...
def run_backtest(
    ...
    for index in range(strategy.minimum_bars_required, len(bars) + 1):
        ...
        if position_qty != 0:
            position = Position(
                ...
                qty=position_qty,
                market_value=abs(position_qty) * latest_bar.close,
        ...
        signal = strategy.evaluate(
            StrategyContext(
                ...
                earnings_releases=tuple(
                    release
                    for release in earnings_releases
                    if release.observed_at <= latest_bar.timestamp and release.event_id not in runtime_state.traded_event_ids
                ),
            )
        )
        runtime_state.apply_signal_updates(instrument.symbol, signal)
        if signal.action in {SignalAction.BUY, SignalAction.SHORT} and runtime_state.kill_switch_active:
            continue
        if signal.action in {SignalAction.BUY, SignalAction.SHORT} and not runtime_state.can_enter(instrument, latest_bar.timestamp):
            continue
        ...
        if plan is None:
            if managed_position is not None and signal.stop_price is not None:
                ...
                managed_position.lowest_price = signal.lowest_price or managed_position.lowest_price
        ...
        if signal.action is SignalAction.BUY and plan.side is OrderSide.BUY and plan.qty is not None:
            ...
            managed_position = ManagedPosition(
                ...
                lowest_price=signal.lowest_price or latest_bar.low,
        ...
        if signal.action is SignalAction.SHORT and plan.side is OrderSide.SELL and plan.qty is not None:
            notional = plan.qty * latest_bar.close
            cash += notional
            position_qty -= plan.qty
            managed_position = ManagedPosition(
    ...
    if position_qty != 0 and managed_position is not None:
        latest_bar = bars[-1]
        qty_to_flatten = abs(position_qty)
        if position_qty > 0:
            cash += qty_to_flatten * latest_bar.close
            trades.append(
                BacktestTrade(
                    timestamp=latest_bar.timestamp,
                    side=OrderSide.SELL,
                    price=latest_bar.close,
                    qty=qty_to_flatten,
                    notional=qty_to_flatten * latest_bar.close,
